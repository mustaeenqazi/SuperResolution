...
def get_train_data():
    ...
    def generator_train():
        for img,im in zip(train_lr_imgs,train_hr_imgs):
            yield img,im
    def _mapping_func_train_(img,im):
        hr_patch = tf.image.random_crop(im, [384, 384, 3])
        ...
        print(lr_patch.dtype)
        print(hr_patch.dtype)
        return lr_patch, hr_patch
    
    train_ds = tf.data.Dataset.from_generator(generator_train, output_types=(tf.float32, tf.float32) )
    ##output_shapes=([96,96,3], [384,384,3])

    train_ds = train_ds.map(_mapping_func_train_,num_parallel_calls=multiprocessing.cpu_count()) #, #num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def evaluate():
    ...
    #valid_lr_img = get_imgs_fn('0050.png', 'test_data/')  # if you want to test your own image
    valid_lr_img = np.float32(valid_lr_img)[:,:,:3] #this line is used to remove alphalayer from images
